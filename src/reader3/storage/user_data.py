...
class UserStateRepository(Protocol):
    """Storage boundary for reader state and local app metadata."""
...
class UserDataManager:
    """Manages user data persistence with debounced SQLite writes."""
    
    DEBOUNCE_SECONDS = 2.0  # Flush after 2 seconds of inactivity
    DB_FILENAME = "reader3.sqlite3"

    def __init__(self, data_dir: str):
        self.data_dir = data_dir
        self.data_file = os.path.join(data_dir, "user_data.json")
        self.db_file = os.path.join(data_dir, self.DB_FILENAME)
    ...
    def _connect(self) -> sqlite3.Connection:
        """Open a SQLite connection for user data persistence."""
    ...
    def _ensure_schema(self):
        ...
        with self._managed_connection() as conn:
            conn.executescript(
                ...
                create table if not exists copied_pages (
                    book_id text primary key,
                    items_json text not null
                );

                create index if not exists idx_highlights_book
    ...
    def _data_from_raw(self, raw: dict) -> UserData:
        """Reconstruct UserData from the legacy JSON shape."""
        return UserData(
            highlights={
    ...
    def _load_legacy_json(self) -> UserData:
        """Load legacy JSON user data for one-time migration."""
        if not os.path.exists(self.data_file):
            return UserData()
        try:
            with open(self.data_file, 'r', encoding='utf-8') as f:
                raw = json.load(f)
            return self._data_from_raw(raw)
    ...
    def _load_from_sqlite(self) -> UserData:
        ...
        with self._managed_connection() as conn:
            ...
            for row in conn.execute("select * from bookmarks order by created_at"):
                bookmark = Bookmark(**dict(row))
                data.bookmarks.setdefault(bookmark.book_id, []).append(bookmark)

            for row in conn.execute("select * from reading_progress"):
                progress = ReadingProgress(**dict(row))
                data.progress[progress.book_id] = progress

            for row in conn.execute("select * from chapter_progress"):
                book_id = row["book_id"]
                chapter_index = int(row["chapter_index"])
                data.chapter_progress.setdefault(book_id, {})[chapter_index] = (
                    row["progress_percent"]
                )

            for row in conn.execute(
                "select query, book_id, timestamp, results_count "
                "from search_history order by position"
            ):
                data.search_history.append(SearchQuery(**dict(row)))

            for row in conn.execute(
                "select id, book_id, book_title, chapter_index, chapter_title, "
                "start_time, end_time, duration_seconds, pages_read, "
                "scroll_position from reading_sessions order by position"
            ):
                data.reading_sessions.append(ReadingSession(**dict(row)))

            for row in conn.execute("select * from vocabulary order by created_at"):
                word = VocabularyWord(**dict(row))
                data.vocabulary.setdefault(word.book_id, []).append(word)

            for row in conn.execute("select * from annotations order by created_at"):
                values = dict(row)
                values["tags"] = json.loads(values.pop("tags_json") or "[]")
                annotation = Annotation(**values)
                data.annotations.setdefault(annotation.book_id, []).append(annotation)

            for row in conn.execute("select * from collections order by sort_order"):
                values = dict(row)
                values["book_ids"] = json.loads(
                    values.pop("book_ids_json") or "[]"
                )
                values["is_smart"] = bool(values["is_smart"])
                data.collections.append(Collection(**values))

            prefs = conn.execute(
                "select * from reader_preferences where id = 1"
            ).fetchone()
            if prefs:
                pref_values = dict(prefs)
                pref_values.pop("id", None)
                pref_values["reduced_motion"] = bool(
                    pref_values["reduced_motion"]
                )
                pref_values["high_contrast"] = bool(
                    pref_values["high_contrast"]
                )
                data.reader_preferences = ReaderPreferences(**pref_values)

            for row in conn.execute("select * from book_fonts"):
                data.book_fonts[row["book_id"]] = row["font_family"]

            for row in conn.execute("select * from copied_pages"):
                data.copied_pages[row["book_id"]] = json.loads(
                    row["items_json"] or "[]"
                )
    ...
    def _write_to_sqlite(self, data: UserData):
        ...
        with self._managed_connection() as conn:
            ...
            for highlights in data.highlights.values():
                conn.executemany(
                    "insert into highlights values "
                    "(:id, :book_id, :chapter_index, :text, :color, :note, "
                    ":start_offset, :end_offset, :created_at)",
                    [asdict(highlight) for highlight in highlights],
                )

            for bookmarks in data.bookmarks.values():
                conn.executemany(
                    "insert into bookmarks values "
                    "(:id, :book_id, :chapter_index, :scroll_position, "
                    ":title, :note, :created_at)",
                    [asdict(bookmark) for bookmark in bookmarks],
                )

            conn.executemany(
                "insert into reading_progress values "
                "(:book_id, :chapter_index, :scroll_position, :last_read, "
                ":total_chapters, :reading_time_seconds)",
                [asdict(progress) for progress in data.progress.values()],
            )
            ...
            chapter_rows = [
                ...
                for book_id, chapters in data.chapter_progress.items()
                for chapter_index, progress_percent in chapters.items()
            ]
            conn.executemany(
            ...
            conn.executemany(
                ...
                "(:position, :query, :book_id, :timestamp, :results_count)",
                [
                    {"position": idx, **asdict(query)}
                    for idx, query in enumerate(data.search_history)
                ],
            )

            conn.executemany(
                "insert into reading_sessions values "
                "(:position, :id, :book_id, :book_title, :chapter_index, "
                ":chapter_title, :start_time, :end_time, :duration_seconds, "
                ":pages_read, :scroll_position)",
                [
                    {"position": idx, **asdict(session)}
                    for idx, session in enumerate(data.reading_sessions)
                ],
            )
            ...
            for words in data.vocabulary.values():
                conn.executemany(
                    ...
                    "(:id, :book_id, :word, :definition, :phonetic, "
                    ":part_of_speech, :example, :chapter_index, :context, "
                    ":created_at, :reviewed_count)",
                    [asdict(word) for word in words],
                )
            ...
            for annotations in data.annotations.values():
                for annotation in annotations:
                    ...
                    values["tags_json"] = json.dumps(values.pop("tags"))
                    annotation_rows.append(values)
            conn.executemany(
                "insert into annotations values "
                "(:id, :book_id, :chapter_index, :note_text, :highlight_id, "
                ":bookmark_id, :position_offset, :created_at, :updated_at, "
                ":tags_json)",
                annotation_rows,
            )
            ...
            for collection in data.collections:
                ...
                values["book_ids_json"] = json.dumps(values.pop("book_ids"))
                values["is_smart"] = int(values["is_smart"])
                collection_rows.append(values)
            conn.executemany(
                "insert into collections values "
                "(:id, :name, :description, :icon, :color, :book_ids_json, "
                ":created_at, :updated_at, :is_smart, :sort_order)",
                collection_rows,
            )

            prefs = asdict(data.reader_preferences)
            prefs["id"] = 1
            prefs["reduced_motion"] = int(prefs["reduced_motion"])
            prefs["high_contrast"] = int(prefs["high_contrast"])
            conn.execute(
            ...
            conn.executemany(
                "insert into book_fonts values (:book_id, :font_family)",
                [
                    {"book_id": book_id, "font_family": font_family}
                    for book_id, font_family in data.book_fonts.items()
                ],
            )

            conn.executemany(
                "insert into copied_pages values (:book_id, :items_json)",
                [
                    {
                        "book_id": book_id,
                        "items_json": json.dumps(items),
                    }
                    for book_id, items in data.copied_pages.items()
                ],
            )

            conn.execute(
                "insert into app_meta values ('version', ?)",
                (data.version,),
            )
    ...
    def load(self) -> UserData:
        ...
        if not self._has_sqlite_data() and os.path.exists(self.data_file):
            self._data = self._load_legacy_json()
            self._write_to_sqlite(self._data)
        else:
